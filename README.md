# FIFA-World-Cup-Match-Outcome-Prediction-Machine-Learning-Model
----------
This is a supervised ML model that predicts match winners in the FIFA world cup using historic World Cup data. I built it in python and used the following libraries:

-Pandas: for data processing.

-Scikit-Learn: for model training and evaluating Linear Regression and Random Forest models.

I applied feature engineering and model evaluation techniques to improve prediction accueacy from an initial 53% to 60.6%.
